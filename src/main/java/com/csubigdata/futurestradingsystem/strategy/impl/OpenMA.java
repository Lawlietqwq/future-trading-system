...
@ToString
@NoArgsConstructor
public class OpenMA implements TradingExecutable {
    ...
    public OpenMA(String code, int uid, int modelId, String xinyiAccount, String xinyiPwd, String tradingAccount, String tradingPwd, String company, int lot, List<Parameter> openParams, boolean bkOrSk, ModelStateEnum modelState, ReentrantLock lock){
        this.code = code;
        this.uid = uid;
        ...
        this.lot = lot;
        this.shortParam = (int)openParams.get(0).getParamValue();
        this.longParam = (int)openParams.get(1).getParamValue();
        this.bkOrSk = bkOrSk;
        ...
        if (bkOrSk){
            ...
            if (longMA.get(longSize-2) >= shortMA.get(shortSize-2) && longMA.get(longSize-1) <= shortMA.get(shortSize-1)){
                if(!modelInstance.isFinished() && lock.tryLock()) {
                    ...
                    int code = HttpClientUtil.doPostParams(remoteVO, Constants.OPEN_URL);
                    if (code == 200) {
                        modelInstance.setModelStatus(ModelStateEnum.holding);
                        modelService.open(modelId);
                        lock.unlock();
                    } else {
                        lock.unlock();
                        CommonException.fail(ResultTypeEnum.OPEN_ERROR);
                    }
                }
            }
        } else {
